...
// Este script recibe datos del formulario de pago y los guarda en Google Sheets
...
function doPost(e) {
  try {
    ...
    // ID de la hoja de cálculo (reemplazar con tu ID real)
    const SPREADSHEET_ID = '15HmHDKtyEIMiZRu7U5Zqp3TYwAp05qd6CYphiwicGJY';
    
    // Abrir la hoja de cálculo
    const spreadsheet = SpreadsheetApp.openById(SPREADSHEET_ID);
    
    // Obtener o crear la hoja "Base de Datos Clientes Venture Zone"
    let sheet = spreadsheet.getSheetByName('Base de Datos Clientes Venture Zone');
    if (!sheet) {
      sheet = spreadsheet.insertSheet('Base de Datos Clientes Venture Zone');
      setupHeaders(sheet);
    }
...
function cleanOldData() {
  try {
    const SPREADSHEET_ID = '15HmHDKtyEIMiZRu7U5Zqp3TYwAp05qd6CYphiwicGJY';
    const spreadsheet = SpreadsheetApp.openById(SPREADSHEET_ID);
    const sheet = spreadsheet.getSheetByName('Base de Datos Clientes Venture Zone');
