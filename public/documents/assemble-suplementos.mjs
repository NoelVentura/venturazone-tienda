...
const main = fs.readFileSync(path.join(__dirname, 'suplementos-main.html'), 'utf8');
const suplementos = JSON.parse(fs.readFileSync(path.join(__dirname, 'suplementos-data.json'), 'utf8'));
const filterLabels = JSON.parse(fs.readFileSync(path.join(__dirname, 'suplementos-filter-labels.json'), 'utf8'));

const searchProducts = suplementos.map(p => ({
  name: p.flavor ? `${p.name} — ${p.flavor}` : p.name,
...
<head>
    ...
    <meta name="description" content="Venture Zone — Suplementos Hinode. Hiber Hake, H-MEN, SENS HERBALIS y más. Complementos alimenticios originales con envío gratis en compras mayores a $500.">
    <meta name="keywords" content="Venture Zone, suplementos, Hinode, Hiber Hake, H-MEN, SENS HERBALIS, vitaminas, bienestar, Tepic, México">
    <meta name="author" content="Venture Zone">
    <meta name="robots" content="index, follow">
    <link rel="canonical" href="https://venturezone.com/documents/suplementos.html">
    <meta property="og:type" content="website">
    <meta property="og:site_name" content="Venture Zone">
    <meta property="og:title" content="Suplementos Hinode - Venture Zone">
    <meta property="og:description" content="Suplementos Hinode de calidad: Hiber Hake, H-MEN, SENS HERBALIS. Nutrición y bienestar con asesoría personalizada.">
    <meta property="og:image" content="../images/shakefresa.png">
    <meta property="og:url" content="https://venturezone.com/documents/suplementos.html">
    <meta property="og:locale" content="es_MX">
    <meta name="twitter:card" content="summary_large_image">
    <meta name="twitter:title" content="Suplementos Hinode - Venture Zone">
    <meta name="twitter:description" content="Descubre suplementos Hinode originales en Venture Zone. Salud, energía e inmunidad con respaldo profesional.">
    <meta name="twitter:image" content="../images/shakefresa.png">
    <title>Suplementos Hinode - Venture Zone</title>
    <script>
        (function () {
    ...
    <link rel="stylesheet" href="styles-suplementos.css">
</head>
<body>
    ...
    <nav class="site-nav">
        <div class="nav-inner">
            ...
            <ul class="nav-links">
                ...
                <li><a href="areaperfumes.html" class="nav-a">Perfumes</a></li>
                <li><a href="suplementos.html" class="nav-a is-active" aria-current="page">Suplementos</a></li>
        ...
        <div class="mobile-menu" id="mobileMenu">
            ...
            <a href="areaperfumes.html" class="nav-a">Perfumes</a>
            <a href="suplementos.html" class="nav-a is-active" aria-current="page">Suplementos</a>
    ...
    <div class="disclaimer-sticky fu" role="note" aria-live="polite">
        <strong>Este producto no es un medicamento.</strong> No sustituye una dieta balanceada. Consulta con tu médico antes de consumir. <a href="#disclaimer">Aviso legal completo</a>
    </div>
...
const footer = `
    ...
    <a href="https://wa.me/523111213128?text=Hola%20Venture%20Zone%2C%20me%20interesan%20los%20suplementos%20Hinode." class="wa-fab" id="waFab" target="_blank" rel="noopener noreferrer" aria-label="Contactar por WhatsApp">
    ...
    <button type="button" class="compare-fab" id="compareFab" hidden aria-label="Abrir comparador de productos">
        <span aria-hidden="true">⚖</span>
        Comparar (<span id="compareFabCount">0</span> <span id="compareFabLabel">productos</span>)
    </button>

    <div class="compare-modal" id="compareModal" hidden role="dialog" aria-modal="true" aria-labelledby="compareModalTitle">
        <div class="compare-modal-backdrop" id="compareModalBackdrop" aria-hidden="true"></div>
        <div class="compare-modal-box">
            <header class="compare-modal-header">
                <h2 class="h-mix" id="compareModalTitle" style="font-size:1.15rem;">Comparar productos</h2>
                <button type="button" class="compare-modal-close" id="compareModalClose" aria-label="Cerrar comparador">✕</button>
            </header>
            <div class="compare-modal-body" id="compareModalBody" role="region" aria-live="polite"></div>
            <footer class="compare-modal-footer">
                <button type="button" class="btn-s" id="compareModalClear">Vaciar comparación</button>
                <button type="button" class="btn-s" id="compareModalCloseFooter">Cerrar</button>
            </footer>
        </div>
    </div>
    ...
    <div class="search-modal" id="searchModal">
        <div class="search-modal-content">
            <div class="search-modal-header">
                <h3 class="h-mix" style="font-size:1rem;">Buscar Suplementos</h3>
                <button class="search-modal-close" id="searchModalClose">✕</button>
            </div>
            <input type="text" class="search-modal-input" placeholder="Escribe el nombre del suplemento..." id="searchModalInput">
            <div id="searchModalResults"></div>
        </div>
    </div>

    <script>
        const SUPLEMENTOS_DATA = ${JSON.stringify(suplementos)};
        ...
        function initSuplementosHeroVideo() {
            const video = document.getElementById('suplementosHeroVideo');
            ...
            function updateSoundToggle() {
                ...
                videoSoundToggle.hidden = !playing;
                videoSoundToggle.textContent = video.muted ? '🔇' : '🔊';
                videoSoundToggle.setAttribute('aria-label', video.muted ? 'Activar sonido del video' : 'Silenciar video');
                videoSoundToggle.setAttribute('title', video.muted ? 'Activar sonido' : 'Silenciar');
            }
            ...
            video.addEventListener('play', updateSoundToggle);
            video.addEventListener('pause', updateSoundToggle);
            ...
            videoSoundToggle?.addEventListener('click', (e) => {
                ...
                if (!video.muted) video.volume = 0.85;
                if (video.paused) video.play();
                updateSoundToggle();
            });
        }
        initSuplementosHeroVideo();
        ...
        function initScrollAnimations() {
            ...
            document.querySelectorAll('.grid-2, .grid-3, .grid-4, .grid-auto, .family-grid, #suplementosGrid, .featured-grid, .goal-grid, .blog-grid').forEach(container => {
        ...
        const CATALOG_FILTER_LABELS = ${JSON.stringify(filterLabels)};

        const catalogFilterState = {
            gender: [], category: [], goal: [], format: [], price: [], brand: [], certification: [],
            priceMin: 110, priceMax: 599, priceSlider: false,
        };

        let catalogFilterTimer = null;
        const CATALOG_TOTAL = document.querySelectorAll('#suplementosGrid .suplemento-card').length || SUPLEMENTOS_DATA.length;
        ...
        function readFilterStateFromDOM() {
            ...
            catalogFilterState.category = getCheckedValues('category');
            catalogFilterState.goal = getCheckedValues('goal');
            catalogFilterState.format = getCheckedValues('format');
            catalogFilterState.price = getCheckedValues('price');
            catalogFilterState.brand = getCheckedValues('brand');
            catalogFilterState.certification = getCheckedValues('certification');
            catalogFilterState.priceSlider = document.getElementById('filterPriceSliderActive')?.checked || false;
            catalogFilterState.priceMin = parseInt(document.getElementById('filterPriceMin')?.value || '110', 10);
            catalogFilterState.priceMax = parseInt(document.getElementById('filterPriceMax')?.value || '599', 10);
        }
        ...
        function cardMatchesFilters(card) {
            ...
            if (s.category.length) {
                ...
                if (!s.category.some(c => cardCats.includes(c))) return false;
            }
            if (s.goal.length) {
                const cardGoals = (card.dataset.goal || '').split(/\\s+/);
                if (!s.goal.some(g => cardGoals.includes(g))) return false;
            }
            if (s.format.length && !s.format.includes(card.dataset.format)) return false;
            if (s.brand.length && !s.brand.includes(card.dataset.brand)) return false;
            if (s.certification.length) {
                const cardCerts = (card.dataset.certification || '').split(/\\s+/).filter(Boolean);
                if (!s.certification.some(c => cardCerts.includes(c))) return false;
            }
        ...
        function countActiveFilters() {
            let n = catalogFilterState.gender.length + catalogFilterState.category.length
                + catalogFilterState.goal.length + catalogFilterState.format.length
                + catalogFilterState.price.length + catalogFilterState.brand.length
                + catalogFilterState.certification.length;
        ...
        function updateFilterUI(visibleCount) {
            ...
            const grid = document.getElementById('suplementosGrid');
            const activeN = countActiveFilters();

            if (countEl) countEl.innerHTML = 'Mostrando <strong>' + visibleCount + '</strong> de ' + total + ' suplementos';
            ...
            if (noResults) {
                ...
                if (!noResults.hidden) noResults.scrollIntoView({ behavior: motionOK ? 'smooth' : 'auto', block: 'nearest' });
            }
        ...
        function renderActiveFilterTags() {
            ...
            addTags('category', catalogFilterState.category, CATALOG_FILTER_LABELS.category);
            addTags('goal', catalogFilterState.goal, CATALOG_FILTER_LABELS.goal);
            addTags('format', catalogFilterState.format, CATALOG_FILTER_LABELS.format);
            addTags('price', catalogFilterState.price, CATALOG_FILTER_LABELS.price);
            addTags('certification', catalogFilterState.certification, CATALOG_FILTER_LABELS.certification);
        ...
        function updateUrlFromFilters() {
            ...
            if (s.category.length) params.set('categoria', s.category.join(','));
            if (s.goal.length) params.set('objetivo', s.goal.join(','));
            if (s.format.length) params.set('formato', s.format.join(','));
            if (s.price.length) params.set('precio', s.price.join(','));
            if (s.brand.length) params.set('marca', s.brand.join(','));
            if (s.certification.length) params.set('certificacion', s.certification.join(','));
        ...
        function applyFiltersFromUrl() {
            ...
            setChecks('category', 'categoria');
            setChecks('goal', 'objetivo');
            setChecks('format', 'formato');
            setChecks('price', 'precio');
            setChecks('brand', 'marca');
            setChecks('certification', 'certificacion');
        ...
        function applySuplementoFilters(immediate) {
            clearTimeout(catalogFilterTimer);
            const run = () => {
                readFilterStateFromDOM();
                const grid = document.getElementById('suplementosGrid');
                const cards = [...document.querySelectorAll('#suplementosGrid .suplemento-card')];
        ...
        function initSuplementoFilters() {
            const grid = document.getElementById('suplementosGrid');
            if (!grid) return;
            ...
            ['filterPriceMin', 'filterPriceMax'].forEach(id => {
                document.getElementById(id)?.addEventListener('input', () => {
                    updatePriceSliderLabels();
                    if (document.getElementById('filterPriceSliderActive')?.checked) applySuplementoFilters();
                });
            });
            ...
            document.querySelectorAll('[data-filter-category]').forEach(link => {
                link.addEventListener('click', (e) => {
                    e.preventDefault();
                    setFilterCheckboxes('category', [link.dataset.filterCategory]);
                    applySuplementoFilters(true);
                    document.getElementById('coleccion')?.scrollIntoView({ behavior: motionOK ? 'smooth' : 'auto' });
                });
            });

            document.querySelectorAll('[data-filter-goal]').forEach(link => {
                link.addEventListener('click', (e) => {
                    e.preventDefault();
                    setFilterCheckboxes('goal', [link.dataset.filterGoal]);
                    applySuplementoFilters(true);
                    document.getElementById('coleccion')?.scrollIntoView({ behavior: motionOK ? 'smooth' : 'auto' });
                });
            });
        ...
        function initBrandFilter() {
            document.querySelectorAll('.brand-logo-card').forEach(btn => {
                btn.addEventListener('click', () => setBrandFilter(btn.dataset.brand));
            });
        }

        function initProductCompare() {
            const MAX_COMPARE = 3;
            const COMPARE_KEY = 'ventureZoneCompareSup';
            const fab = document.getElementById('compareFab');
            const fabCount = document.getElementById('compareFabCount');
            const fabLabel = document.getElementById('compareFabLabel');
            ...
            const closeFooter = document.getElementById('compareModalCloseFooter');
            const clearBtn = document.getElementById('compareModalClear');
            const sectionOpen = document.getElementById('compareSectionOpen');
            const formatMap = CATALOG_FILTER_LABELS.format || {};
            const certMap = CATALOG_FILTER_LABELS.certification || {};
            ...
            function loadCompare() {
                try {
                    ...
                    if (Array.isArray(saved)) {
                        compareIds = saved
                            .filter(id => SUPLEMENTOS_DATA.some(p => p.id === id))
                            .slice(0, MAX_COMPARE);
                    }
                } catch (_) { /* ignore */ }
            }
            ...
            function ingredientList(p) {
                if (p.ingredientProfile?.items?.length) {
                    return p.ingredientProfile.items.map(i => i.name).join(', ');
                }
                return p.ingredients || '—';
            }

            function syncCheckboxes() {
                document.querySelectorAll('.product-compare-input').forEach(input => {
                    input.checked = compareIds.includes(input.value);
                });
                document.querySelectorAll('.suplemento-card').forEach(card => {
                    const id = card.id.replace('card-', '');
                    card.classList.toggle('is-in-compare', compareIds.includes(id));
                });
            }
            ...
            function updateFab() {
                ...
                if (fab) {
                    fab.hidden = n === 0;
                    fab.setAttribute('aria-label', 'Comparar ' + n + ' producto' + (n !== 1 ? 's' : ''));
                }
                if (fabCount) fabCount.textContent = String(n);
                if (fabLabel) fabLabel.textContent = n === 1 ? 'producto' : 'productos';
            }
            ...
            function lowestPriceIndex(products) {
                if (products.length < 2) return -1;
                const prices = products.map(p => p.price);
                const min = Math.min(...prices);
                if (prices.filter(pr => pr === min).length === products.length) return -1;
                return prices.indexOf(min);
            }

            function buildCompareTable(products) {
                const headers = products.map(p => {
                    const safeName = productLabel(p).replace(/'/g, "\\\\'");
                    return '<th scope="col" class="compare-col-header">' +
                        '<button type="button" class="compare-remove-btn" data-remove-compare="' + p.id + '" aria-label="Quitar ' + productLabel(p) + ' de la comparación">×</button>' +
                        '<img src="../images/' + p.img + '" alt="" width="64" height="64" loading="lazy" decoding="async" class="compare-col-img">' +
                        '<span class="compare-col-name">' + productLabel(p) + '</span>' +
                        '</th>';
                }).join('');
                ...
                const rows = [
                    {
                        label: 'Precio',
                        values: products.map(p => '$' + p.price.toLocaleString('es-MX')),
                        isPrice: true,
                    },
                    {
                        label: 'Ingredientes',
                        values: products.map(ingredientList),
                    },
                    {
                        label: 'Formato',
                        values: products.map(p => formatMap[p.format] || p.format),
                    },
                    {
                        label: 'Dosis / modo de uso',
                        values: products.map(p => p.usage || '—'),
                    },
                    {
                        label: 'Beneficios',
                        values: products.map(p => (p.benefits || []).join(' · ')),
                    },
                    {
                        label: 'Certificaciones',
                        values: products.map(p => formatCerts(p.certifications)),
                    },
                ];

                const cheapIdx = lowestPriceIndex(products);

                let body = '';
                rows.forEach(row => {
                    const differs = rowDiffers(row.values);
                    body += '<tr class="' + (differs ? 'compare-row--diff' : '') + '">';
                    body += '<th scope="row" class="compare-row-label">' + row.label + '</th>';
                    row.values.forEach((val, i) => {
                        let cls = 'compare-cell';
                        if (differs) cls += ' compare-cell--diff';
                        if (row.isPrice && i === cheapIdx) cls += ' compare-cell--best';
                        body += '<td class="' + cls + '">' + val + '</td>';
                    });
                    body += '</tr>';
                });

                const cartRow = '<tr class="compare-row--actions"><th scope="row" class="compare-row-label">Acción</th>' +
                    products.map(p => {
                        const safeCart = p.cartName.replace(/'/g, "\\\\'");
                        return '<td class="compare-cell compare-cell--action">' +
                            '<button type="button" class="btn-p compare-add-cart-btn" onclick="addToCart(\\'' + safeCart + '\\', ' + p.price + ', \\'' + p.id + '\\')">Agregar al carrito</button>' +
                            '</td>';
                    }).join('') + '</tr>';

                return '<div class="compare-table-scroll">' +
                    '<table class="compare-table">' +
                    '<thead><tr><th scope="col" class="compare-corner"></th>' + headers + '</tr></thead>' +
                    '<tbody>' + body + cartRow + '</tbody>' +
                    '</table></div>' +
                    '<p class="compare-legend sec-label"><span class="compare-legend-swatch compare-legend-swatch--diff"></span> Diferencia entre productos &nbsp;·&nbsp; <span class="compare-legend-swatch compare-legend-swatch--best"></span> Mejor precio</p>';
            }

            function renderModal() {
                if (!modalBody) return;
                if (compareIds.length < 2) {
                    modalBody.innerHTML = '<p class="ui-text" style="text-align:center;padding:32px 16px;">Selecciona al menos <strong>2 productos</strong> para comparar. Puedes elegir hasta 3 desde el catálogo.</p>';
                    return;
                }
                const products = compareIds.map(id => SUPLEMENTOS_DATA.find(p => p.id === id)).filter(Boolean);
                modalBody.innerHTML = buildCompareTable(products);
                modalBody.querySelectorAll('[data-remove-compare]').forEach(btn => {
                    btn.addEventListener('click', () => removeFromCompare(btn.dataset.removeCompare));
                });
            }
            ...
            function removeFromCompare(id) {
                ...
                updateFab();
                renderModal();
                if (compareIds.length === 0) closeCompareModal();
            }
            ...
            document.querySelectorAll('.compare-card-check').forEach(label => {
            ...
            sectionOpen?.addEventListener('click', openCompareModal);
            closeBtn?.addEventListener('click', closeCompareModal);
            closeFooter?.addEventListener('click', closeCompareModal);
            backdrop?.addEventListener('click', closeCompareModal);
            clearBtn?.addEventListener('click', () => { clearCompare(); closeCompareModal(); });

            document.addEventListener('keydown', (e) => {
                if (e.key === 'Escape' && modal && !modal.hidden) closeCompareModal();
            });
        ...
        function initContactForm() {
            ...
            const rules = {
                ...
                objetivoSalud: { validate: v => v.trim().length > 0, message: 'Selecciona tu objetivo de salud.' },
        ...
        function sendWhatsApp(e) {
            ...
            const objetivoSalud = document.getElementById('objetivoSalud').value;
            ...
            let text = \`Hola Venture Zone, solicito asesoría en suplementos Hinode.\\n\\n\`;
            text += \`👤 Nombre: \${nombre}\\n\`;
            text += \`📞 Teléfono: \${telefono}\\n\`;
            if (email) text += \`📧 Email: \${email}\\n\`;
            text += \`🎯 Objetivo de salud: \${objetivoSalud}\\n\`;
            if (presupuesto) text += \`💰 Presupuesto: \${presupuesto}\\n\`;
            text += \`👥 Para quién: \${paraQuien}\\n\`;
            if (productoInteres) text += \`💊 Producto de interés: \${productoInteres}\\n\`;
            if (mensaje) text += \`\\n💬 Mensaje:\\n\${mensaje}\\n\`;
        ...
        function activateIngredientTab(productId) {
            document.querySelectorAll('.ingredient-tab').forEach(tab => {
                const active = tab.dataset.ingredientProduct === productId;
                tab.classList.toggle('is-active', active);
                tab.setAttribute('aria-selected', active ? 'true' : 'false');
            });
            document.querySelectorAll('.ingredient-panel').forEach(panel => {
                const active = panel.id === 'ingredient-panel-' + productId;
        ...
        function initIngredientTabs() {
            ...
            tabs.forEach(tab => {
                tab.addEventListener('click', () => {
                    activateIngredientTab(tab.dataset.ingredientProduct);
                    document.getElementById('ingredient-panel-' + tab.dataset.ingredientProduct)
                        ?.scrollIntoView({ behavior: motionOK ? 'smooth' : 'auto', block: 'nearest' });
                });
            });
        }

        function scrollToProduct(id) {
            const section = document.getElementById('detalle-ingredientes');
            if (section) {
                activateIngredientTab(id);
                const navH = document.querySelector('.site-nav')?.offsetHeight || 68;
                const top = section.getBoundingClientRect().top + window.scrollY - navH - 16;
                window.scrollTo({ top, behavior: motionOK ? 'smooth' : 'auto' });
                const panel = document.getElementById('ingredient-panel-' + id);
                panel?.setAttribute('tabindex', '-1');
                panel?.focus({ preventScroll: true });
                panel?.classList.add('ingredient-panel--highlight');
                setTimeout(() => panel?.classList.remove('ingredient-panel--highlight'), 2200);
                return;
            }
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function performSearch(query) {
                ...
                searchModalResults.innerHTML = filtered.length === 0 ? '<div class="no-results">No se encontraron suplementos</div>' :
            ...
            window.selectProductFromModal = function(productName) {
                ...
                document.querySelectorAll('.filter-input:checked').forEach(el => { el.checked = false; });
                let visible = 0;
                document.querySelectorAll('#suplementosGrid .suplemento-card').forEach(card => {
                ...
                updateFilterUI(visible);
                const target = document.querySelector('#suplementosGrid .suplemento-card:not(.is-hidden)');
...
fs.writeFileSync(path.join(__dirname, 'suplementos.html'), out, 'utf8');
console.log('Generated: suplementos.html (' + out.length + ' bytes)');
